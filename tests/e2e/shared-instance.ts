 ...
 *   VERSIONIQ_E2E_ALLOW_SHARED_INSTANCE  this app only
 *
...
export const APP_ID = 'versioniq'

/** The app-specific opt-in variable. Holds an origin, not a boolean. */
export const SHARED_INSTANCE_FLAG = 'VERSIONIQ_E2E_ALLOW_SHARED_INSTANCE'
 ...
 *
 * Resolution order:
 *  1. `VERSIONIQ_E2E_OCC`, a complete prefix, for any rig shape the guesses
 *     below do not cover.
 *  2. `VERSIONIQ_E2E_CONTAINER` or `NEXTCLOUD_CONTAINER`, a container name,
...
export function occPrefix(env: NodeJS.ProcessEnv = process.env): string[] {
	const explicit = (env.VERSIONIQ_E2E_OCC ?? '').trim()
